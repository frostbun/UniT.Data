...
namespace UniT.Data.Storages
{
    ...
    public class FileBinaryStorage : IWritableStorage
    {
        ...
        {
            return File.ReadAllBytes(GetPath(key));
        }
        ...
        {
            return File.ReadAllBytesAsync(GetPath(key)).ToCoroutine(callback);
        }
        ...
        IEnumerator IWritableStorage.WriteAsync(string key, object value, Action? callback, IProgress<float>? progress)
        {
            var path = GetPath(key);
            Directory.CreateDirectory(Path.GetDirectoryName(path)!);
            return File.WriteAllBytesAsync(path, (byte[])value).ToCoroutine(callback);
        }
        #endif
